...
function StockOut() {
  ...
  const handleDispatch = async (e) => {
    ...
    if (qty > checkoutItem.currentStock) {
      return toast.error(`You only have ${checkoutItem.currentStock} in stock! You can't dispatch ${qty}.`);
    }

    const toastId = toast.loading(`Dispatching ${qty}x ${checkoutItem.partName}...`);
    ...
    try {
      const payload = {
        ...
        type: 'OUT',
        quantity: qty,
        referenceNumber: reference || 'Quick Dispatch',
      ...
      if (response.success) {
        ...
        // ✨ FIX: Use sellingPrice for outgoing receipts!
        setReceiptData({
          partName: checkoutItem.partName,
          sku: checkoutItem.sku,
          type: 'STOCK OUT',
          quantity: qty,
          price: checkoutItem.sellingPrice || 0, // Added sellingPrice
          totalAmount: (checkoutItem.sellingPrice || 0) * qty, // Added calculation
          referenceNumber: payload.referenceNumber,
          newStockLevel: response.data?.newStockLevel || (checkoutItem.currentStock - qty),
        ...
        toast.error(response.message || "Failed to dispatch stock.", { id: toastId });
      }
    } catch (error) {
    ...
    <div className="p-8 bg-gray-50 min-h-screen print:p-0 print:bg-white">
      ...
      <div className="print:hidden mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Fast Stock Out</h1>
            <p className="text-gray-500 mt-1">Quickly dispatch items from your inventory</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredInventory.map((item) => (
              <div 
                ...
                className={`bg-white rounded-xl shadow-sm border p-6 transition duration-200 ${
                  item.currentStock > 0 
              ...
              >
                ...
                <div className="flex justify-between items-end border-t border-gray-100 pt-4">
                  ...
                  <div className="text-right">
                    {/* ✨ FIX: Show Selling Price */}
                    <p className="text-xs text-gray-500 uppercase tracking-wider font-bold mb-1">Selling Price</p>
                    <p className="font-bold text-blue-600">₹{item.sellingPrice?.toLocaleString('en-IN') || 0}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* --- CHECKOUT MODAL (Hidden when printing) --- */}
      {checkoutItem && (
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            <div className="bg-blue-600 p-6 flex justify-between items-start">
              <div>
                ...
                <p className="text-blue-200 text-sm mt-1">{checkoutItem.partName}</p>
              </div>
            ...
            <form onSubmit={handleDispatch} className="p-6">
              <div className="bg-blue-50 text-blue-800 p-4 rounded-xl mb-6 flex justify-between items-center border border-blue-100">
                <span className="font-medium">Currently Available:</span>
                <span className="text-xl font-black">{checkoutItem.currentStock} Units</span>
              </div>
              <div className="space-y-4">
                <div>
                  <label className="block text-gray-700 font-bold mb-2 text-sm">Quantity to Dispatch <span className="text-red-500">*</span></label>
                  <input type="number" min="1" max={checkoutItem.currentStock} value={checkoutQty} onChange={(e) => setCheckoutQty(e.target.value)} required autoFocus className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
                  <label className="block text-gray-700 font-bold mb-2 text-sm">Reference Number</label>
                  <input type="text" value={reference} onChange={(e) => setReference(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
              </div>
              <div className="mt-8 flex gap-3">
                <button type="submit" className="flex-1 bg-blue-600 text-white font-bold py-3 px-4 rounded-xl hover:bg-blue-700 transition shadow-md">Confirm Dispatch</button>
                <button type="button" onClick={() => setCheckoutItem(null)} className="bg-gray-100 text-gray-700 font-bold py-3 px-6 rounded-xl hover:bg-gray-200 transition">Cancel</button>
              </div>
            </form>
          </div>
        </div>
            ...
            <div className="p-8 border-b-2 border-dashed border-gray-200 print:border-none">
              ...
              <div className="space-y-4 text-sm text-gray-700">
                <div className="flex justify-between border-b border-gray-100 pb-2">
                  ...
                  <span className="font-bold text-orange-600">{receiptData.type}</span>
                </div>
                <div className="flex justify-between border-b border-gray-100 pb-2">
                ...
                {/* ✨ FIX: Displays Unit Price & Total Amount using sellingPrice! */}
                <div className="flex justify-between border-b border-gray-100 pb-2">
                ...
                <div className="flex justify-between border-b border-gray-100 pb-2">
                  <span className="font-semibold text-gray-500">Qty Dispatched</span>
                  <span className="font-black text-lg text-gray-800">x {receiptData.quantity}</span>
                </div>
                ...
                <div className="flex justify-between bg-gray-50 p-3 rounded-lg border border-gray-200 print:border-none print:bg-transparent print:p-0 pt-2">
                  <span className="font-bold text-gray-800 uppercase">Total Amount</span>
                  <span className="font-black text-xl text-blue-600">₹{receiptData.totalAmount.toLocaleString('en-IN')}</span>
                </div>
